import type { DocumentNode, Node } from 'ultrahtml';

import { parse, render } from 'ultrahtml';

/// <reference types="astro/astro-jsx" />

export type SVGAttributes = Omit<astroHTML.JSX.SVGAttributes, 'client:list' | 'set:text' | 'set:html' | 'is:raw'>;

const EMPTY_STRING_ERR = '`svgSource` must have content';
const MUST_START_WITH_SVG = '`svgSource` must begin with `<svg`';

export async function overrideSvgAttributes(svgSource: string, attributeOverrides: SVGAttributes = {}): Promise<string> {
	if (!svgSource) {
